...
k1_slider = Slider(start=0, end=1.1, value=0.5, step=0.1, title="Upper tank upper discharge coefficient")
k2_slider = Slider(start=0, end=1.1, value=0.2, step=0.1, title="Upper tank lower discharge coefficient")
k3_slider = Slider(start=0, end=1.5, value=0.01, step=0.1, title="Percolation to lower tank coefficient")
k4_slider = Slider(start=0, end=1.1, value=0.1, step=0.1, title="Lower tank discharge coefficient")
d1_txt = TextInput(value="10", title="Upper tank upper discharge position (m):")
d2_txt = TextInput(value="20", title="Upper tank lower discharge position (m):")
rfrc_slider = Slider(start=0.8, end=1.2, value=1, step=0.1, title="Rainfall")
ecorr_slider = Slider(start=0.8, end=1.2, value=1, step=0.1, title="Error Correction")

dr_txt = TextInput(value="20", title="Number of hours in time step (s):")
area_txt = TextInput(value="20", title="Catchment Area (km^2):")

################################# INITIAL WATER STATES ##################################################
s1 = TextInput(value="10", title="Top tank Water Level (m):")
s2 = TextInput(value="10", title="Bottom tank Water Level (m):")
...
INITIAL_STATES = [float(s1.value), float(s2.value)]
INITIAL_Q = 1.0
...
def _step(prec, evap, st, param, extra_param):
    ...
    d1 = float(d1.value)
    d2 = float(d2.value)
    rfcf = float(rfcf.value)
    ecorr = float(ecorr.value)
